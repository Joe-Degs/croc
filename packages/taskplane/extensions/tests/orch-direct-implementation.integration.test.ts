...
function runAllTests(): void {
	...
	// 2) computeResumePoint should NOT re-queue mark-failed tasks as pending.
	{
		...
		assert(
			!resumePoint.pendingTaskIds.includes("TS-200"),
			"mark-failed task is not re-queued as pending",
		);
	...
	// 4) hasTaskDoneMarker checks archived path fallback.
	{
		...
		try {
			...
			assert(
				hasTaskDoneMarker(taskFolder),
				"archived .DONE marker is detected from original task folder path",
			);
		} finally {
	...
	// ── 6) Orch branch creation: failure path (branch already exists) ──
	{
		...
		try {
			...
			assert(
				batchState.phase === "failed",
				"batch state phase set to 'failed' on branch creation failure",
			);
	...
	// cannot leak orphan branches.
	{
		...
		const preflightReturnPos = engineSource.indexOf(
			'batchState.errors.push("Preflight check failed")',
		);
		const discoveryReturnPos = engineSource.indexOf(
			'batchState.errors.push("Discovery had fatal errors',
		);
		...
		const branchCreationPos = engineSource.indexOf(
			'runGit(["branch", orchBranch, batchState.baseBranch]',
		);
	...
	// created when the user is on a detached HEAD.
	{
		...
		try {
			...
			assert(
				batchState.phase === "failed",
				"batch fails on detached HEAD before orch branch creation",
			);
			...
			assert(
				!branchList.includes("orch/"),
				"no orch/ branches exist in repo after detached HEAD rejection",
			);
	...
	// 5) engine.ts passes orchBranch (not baseBranch) to executeWave and mergeWaveByRepo
	{
		...
		const executeWaveBlock =
			engineSource.match(/const waveResult = await executeWave\([\s\S]*?\);/)?.[0] ?? "";
		assert(
		...
		const mergeCallRegex =
			/mergeWaveByRepo\(\s*waveResult\.allocatedLanes[\s\S]*?batchState\.orchBranch/;
		assert(
			mergeCallRegex.test(engineSource),
		...
		const resetBlock =
			...
			"";
		assert(
			resetBlock.includes("batchState.orchBranch"),
		...
		// TP-029: Now iterates encounteredRepoRoots with per-repo target branch.
		const cleanupBlock =
			engineSource.match(/Phase 3: Cleanup[\s\S]*?targetBranch = batchState\.\w+/)?.[0] ?? "";
		assert(
	...
	// 6) resume.ts mirrors engine.ts orchBranch routing
	{
		...
		const resumeExecBlock =
			resumeSource.match(/const waveResult = await executeWave\([\s\S]*?\);/)?.[0] ?? "";
		assert(
		...
		const waveMergeRegex =
			/mergeWaveByRepo\(\s*waveResult\.allocatedLanes[\s\S]*?batchState\.orchBranch/;
		assert(
			waveMergeRegex.test(resumeSource),
			"resume.ts wave mergeWaveByRepo() receives batchState.orchBranch",
		);

		// Re-exec merge also uses orchBranch
		const reExecMergeRegex =
			/reExecAllocatedLanes[\s\S]*?mergeWaveByRepo\([\s\S]*?batchState\.orchBranch/;
		assert(
	...
	// 7) resume.ts has orchBranch empty-guard for pre-TP-022 persisted states
	{
		...
		assert(
			resumeSource.includes("has no orch branch"),
	...
	// 8) resolveBaseBranch in waves.ts: repo mode returns passed-in branch, workspace mode detects per-repo
	{
		...
		assert(
			wavesSource.includes("export function resolveBaseBranch"),
	...
	// 10) R006: resolveBaseBranch throws for orch branch fallback in workspace mode
	{
		...
		try {
			...
			assert(
				e.message.includes("defaultBranch"),
				"error message mentions defaultBranch configuration",
			);
		}
		assert(
			threwForOrchFallback,
	...
	// 13) merge.ts update-ref success path logs correctly
	{
		...
		const successLog =
			mergeSource.match(/`updated \$\{targetBranch\} ref to merge result`/)?.[0] ?? "";
		assert(
			successLog.length > 0,
	...
	// 14) merge.ts workspace-mode safety: checked-out branch uses ff-only, not update-ref
	{
		...
		assert(
			checkedOutIdx > 0 && elseIdx > checkedOutIdx,
		...
		assert(
			advancementBlock.includes("workspace mode"),
	...
	// 15) Behavioral: update-ref advances non-checked-out branch (orch branch path)
	{
		...
		try {
			...
			execSync(`git branch ${tempBranch} ${orchBranch}`, {
				cwd: repoDir,
				encoding: "utf-8",
				stdio: "pipe",
			});
			...
			execSync(`git worktree add "${wtDir}" ${tempBranch}`, {
				cwd: repoDir,
				encoding: "utf-8",
				stdio: "pipe",
			});
			...
			execSync('git commit -m "merge: wave 1 lane 1"', {
				cwd: wtDir,
				encoding: "utf-8",
				stdio: "pipe",
			});
	...
	// 16) Behavioral: update-ref compare-and-swap rejects stale old OID
	{
		...
		try {
			...
			execSync(`git worktree add "${wtDir}" ${orchBranch}`, {
				cwd: repoDir,
				encoding: "utf-8",
				stdio: "pipe",
			});
			...
			execSync(`git worktree add "${wtDir2}" ${tempBranch}`, {
				cwd: repoDir,
				encoding: "utf-8",
				stdio: "pipe",
			});
			...
			execSync(`git worktree remove "${wtDir2}" --force`, {
				cwd: repoDir,
				encoding: "utf-8",
				stdio: "pipe",
			});
	...
	// 17) Behavioral: ff-only advances checked-out branch + working tree (workspace mode path)
	{
		...
		try {
			...
			execSync("git add -A", { cwd: wtDir, encoding: "utf-8", stdio: "pipe" });
			execSync('git commit -m "workspace merge commit"', {
				cwd: wtDir,
				encoding: "utf-8",
				stdio: "pipe",
			});
	...
	// 18) Behavioral: auto-integration fast-forwards baseBranch to orchBranch (update-ref path)
	{
		...
		try {
			...
			execSync("git add -A", { cwd: wtDir, encoding: "utf-8", stdio: "pipe" });
			execSync('git commit -m "task: completed work"', {
				cwd: wtDir,
				encoding: "utf-8",
				stdio: "pipe",
			});
	...
	// 19) Behavioral: auto-integration skips when branches have diverged
	{
		...
		try {
			...
			execSync("git add -A", { cwd: repoDir, encoding: "utf-8", stdio: "pipe" });
			execSync('git commit -m "user: concurrent work"', {
				cwd: repoDir,
				encoding: "utf-8",
				stdio: "pipe",
			});
	...
	// 20) ORCH_MESSAGES integration templates produce correct content
	{
		...
		assert(
			autoSuccessMsg.includes("Auto-integrated"),
			"auto-success message indicates auto-integration",
		);
		assert(autoSuccessMsg.includes("fast-forwarded"), "auto-success message mentions fast-forward");

		// Auto-integration failure message
		const autoFailedMsg = ORCH_MESSAGES.orchIntegrationAutoFailed(
			"orch/op-batch1",
			"main",
			"branches diverged",
		);
	...
	// 21) Structural: engine.ts cleanup preserves orchBranch (no deletion) and uses orchBranch for unmerged detection
	{
		...
		const cleanupSection =
			engineSource.match(/Phase 3: Cleanup[\s\S]*?Post-worktree-removal/)?.[0] ?? "";
		assert(
	...
	{
		console.log(
			"  22) Structural: resume.ts mirrors engine.ts auto-integration + cleanup + messaging",
		);
		const resumeSource = readFileSync(join(__dirname, "..", "taskplane", "resume.ts"), "utf-8");
		const engineSource = readFileSync(join(__dirname, "..", "taskplane", "engine.ts"), "utf-8");
	...
	{
		console.log(
			"  24) Structural: auto-integration gated to terminal phases (completed/failed) only",
		);
		const engineSource = readFileSync(join(__dirname, "..", "taskplane", "engine.ts"), "utf-8");
		const resumeSource = readFileSync(join(__dirname, "..", "taskplane", "resume.ts"), "utf-8");

		// engine.ts: isTerminalPhase gate before auto-integration
		const engineAutoBlock =
			engineSource.match(/Auto-Integration[\s\S]*?orchIntegrationManual/)?.[0] ?? "";
		assert(
		...
		assert(
			engineAutoBlock.includes("isTerminalPhase"),
		...
		assert(
			resumeAutoBlock.includes("isTerminalPhase"),
		...
		// TP-193: regex uses `\s*` between tokens because the formatter wraps long
		// boolean expressions vertically (`if (\n\tisTerminalPhase &&\n\t!preserveWorktreesForResume`).
		const gateRegex = /if \(\s*isTerminalPhase\s*&&\s*!preserveWorktreesForResume/;
		const engineGateIf = engineSource.match(gateRegex);
		assert(
			engineGateIf !== null,
			"engine.ts gates auto-integration with isTerminalPhase in if condition",
		);
		const resumeGateIf = resumeSource.match(gateRegex);
		assert(
			resumeGateIf !== null,
	...
	{
		console.log(
			"  25) Structural: resume.ts resolves per-repo target branch for workspace-mode cleanup",
		);
		const resumeSource = readFileSync(join(__dirname, "..", "taskplane", "resume.ts"), "utf-8");
		...
		const helperFn =
			resumeSource.match(/function resolveRepoIdFromRoot[\s\S]*?return undefined;\s*\}/)?.[0] ?? "";
		assert(
			helperFn.includes("workspaceConfig"),
			"resolveRepoIdFromRoot uses workspaceConfig for reverse lookup",
		);
		assert(
			helperFn.includes("repoConfig.path === repoRoot"),
	...
	// 26) Structural: resume.ts inter-wave reset also uses per-repo target branch
	{
		...
		assert(
			resetSection.includes("resolveRepoIdFromRoot"),
