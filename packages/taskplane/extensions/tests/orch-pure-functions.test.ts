...
function buildDashboardViewModel(batchState: any, monitorState?: any): any {
	...
	if (monitorIsFresh && monitorState) {
		...
		const sortedLanes = [...batchState.currentLanes].sort(
			(a: any, b: any) => a.laneNumber - b.laneNumber,
		);
		for (const lane of sortedLanes) {
			laneCards.push({
				laneNumber: lane.laneNumber,
...
function runAllTests(): void {
...

	{
		...
		// TP-193: Use a regex with `\s*` for `Math.max(0` so the formatter's
		// vertical re-wrapping (Math.max(\n\t\t0, ...)) doesn't break the check.
		assert(/Math\.max\(\s*0\s*,/.test(fnSrc), "computeOrchSummaryCounts: has Math.max(0 for queued");
		assert(
			fnSrc.includes("batchState.skippedTasks"),
			"computeOrchSummaryCounts: subtracts skippedTasks",
		);
...

	{
		...
		assert(
			fnSrc.includes("failedTaskIds.has(dep)"),
			"computeTransitiveDependents: skips failed tasks",
		);
	}

	{
		...
		assert(
			fnSrc.includes("laneNumber - b.laneNumber"),
			"buildDashboardViewModel: sorts by laneNumber",
		);
		assert(
			fnSrc.includes("lane.laneSessionId"),
			"buildDashboardViewModel: uses laneSessionId from allocation",
		);
		...
		assert(
			fnSrc.includes("monitorIsFresh"),
			"buildDashboardViewModel: detects stale monitor data (TP-170)",
		);
...

	{
		...
		assert(
			fnSrc.includes("session ended"),
			"renderLaneCard: softened 'session dead' to 'session ended' (TP-170)",
		);
		assert(
...

	{
		...
		const vm = buildDashboardViewModel(batch, monitor);
		assertEqual(
			vm.laneCards[0].sessionName,
			"orch-henry-api-lane-1",
			"session name reconciled from allocation",
		);
	...
	)(resolve, resolveWorktreeBasePathFn, {
		orchestrator: { worktree_location: defaultWorktreeLocation },
	}) as (prefix: string, laneNumber: number, repoRoot: string, opId: string, config?: any) => string;
...

	{
		...
		assertEqual(
			...
			"subdirectory",
			"DEFAULT_ORCHESTRATOR_CONFIG uses subdirectory",
		);
	...
	}
	{
		...
		const result = resolveSavedBranchCollisionFn(
			"saved/task/lane-1",
			"abc123",
			"def456",
			"2026-03-09T120000",
