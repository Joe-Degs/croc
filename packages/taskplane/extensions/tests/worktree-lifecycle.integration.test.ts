...
describe("5.2 createWorktree — happy path", () => {
	...
	test("handles worktree paths containing spaces", () => {
		...
		assertEqual(
			headBranch,
			"task/test-lane-2-space001",
			"checked out branch in spaced path worktree",
		);
...
describe("5.2 createWorktree — error paths", () => {
	...
	test("WORKTREE_BRANCH_EXISTS for duplicate branch name", () => {
		...
		execSync("git branch task/test-lane-2-test005", {
			cwd: repoDir,
			encoding: "utf-8",
			stdio: "pipe",
		});
...
describe("5.4 removeWorktree — unmerged branch", () => {
	...
	test("force-deletes unmerged branch", () => {
		...
		execSync('git commit -m "worktree-only commit"', {
			cwd: wt.path,
			encoding: "utf-8",
			stdio: "pipe",
		});
...
describe("5.6 listWorktrees — prefix filtering", () => {
	...
	test("ignores non-orchestrator worktrees", () => {
		...
		createWorktree(
			{ laneNumber: 1, batchId: "list002", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);
		...
		execSync(`git worktree remove --force "${otherPath}"`, {
			cwd: repoDir,
			encoding: "utf-8",
			stdio: "pipe",
		});
...
describe("5.6 createLaneWorktrees — bulk creation", () => {
	...
	test("creates N=3 worktrees with correct naming", () => {
		...
		for (let i = 0; i < 3; i++) {
			...
			assertEqual(
				result.worktrees[i].branch,
				`task/test-lane-${i + 1}-bulk001`,
				`lane ${i + 1} branch`,
			);
	...
	test("rolls back on partial failure", () => {
		...
		execSync("git branch task/test-lane-2-bulkfail", {
			cwd: repoDir,
			encoding: "utf-8",
			stdio: "pipe",
		});
		...
		assertEqual(
			...
			"WORKTREE_BRANCH_EXISTS",
			"error code should be WORKTREE_BRANCH_EXISTS",
		);
...
describe("5.6 removeAllWorktrees — bulk removal", () => {
	...
	test("removes all matching worktrees", () => {
		...
		// Create 3 worktrees
		createWorktree(
			{ laneNumber: 1, batchId: "rmall001", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);
		createWorktree(
			{ laneNumber: 2, batchId: "rmall001", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);
		createWorktree(
			{ laneNumber: 3, batchId: "rmall001", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);
...
describe("5.7 Batch-scoped isolation — same opId, different batchIds", () => {
	...
	test("listWorktrees(batchId=A) returns only batch A's lanes", () => {
		...
		createWorktree(
			{ laneNumber: 1, batchId: "batchA", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);
		createWorktree(
			{ laneNumber: 2, batchId: "batchA", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);

		// Create worktrees in batch B (same opId, different batchId)
		createWorktree(
			{ laneNumber: 1, batchId: "batchB", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);
		createWorktree(
			{ laneNumber: 3, batchId: "batchB", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);
	...
	test("batch container is removed when all lanes in it are removed", () => {
		...
		assert(
			...
			"batch container should be removed after all worktrees cleaned up",
		);
...
describe("5.8 Transition compatibility — legacy flat and new nested coexistence", () => {
	...
	test("listWorktrees() without batchId finds both legacy flat and new nested worktrees", () => {
		...
		createWorktree(
			{ laneNumber: 1, batchId: "new001", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);
	...
	test("listWorktrees(batchId=X) does NOT pick up legacy flat worktrees", () => {
		...
		// Create a new nested worktree (batch-scoped)
		createWorktree(
			{ laneNumber: 1, batchId: "new002", baseBranch: "develop", opId: "test", prefix },
			repoDir,
		);
