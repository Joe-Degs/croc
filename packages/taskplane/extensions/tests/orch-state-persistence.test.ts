...
function runAllTests() {
...

	{
		...
		assertEqual(
			...
			"v1 upconverted to v2 schemaVersion",
		);
		...
		assertEqual(
			result.tasks[0].resolvedRepoId,
			undefined,
			"v1 upconvert: task resolvedRepoId is undefined",
		);
...

	{
		...
		assertEqual(
			result.tasks[0].resolvedRepoId,
			"api",
			"v2 workspace: task[0].resolvedRepoId is 'api'",
		);
	...
	{
		console.log(
			"  ▸ validates fixture batch-state-v2-bad-repo-fields.json rejects at first bad field",
		);
...

	{
		...
		const validBase = JSON.parse(loadFixture("batch-state-valid.json"));
		validBase.mergeResults = [
			{ waveIndex: 0, status: "exploded", failedLane: null, failureReason: null },
		];
		assertThrows(
			() => validatePersistedState(validBase),
			"STATE_SCHEMA_INVALID",
	...
	try {
		{
			...
			assertEqual(
				...
				BATCH_STATE_SCHEMA_VERSION,
				"v1 upconverted: schemaVersion is 2",
			);
			...
			assertEqual(
				...
				"/tmp/tasks/TS-001",
				"v1 upconverted: taskFolder preserved",
			);
			...
			assertEqual(
				...
				undefined,
				"v1 upconverted: task resolvedRepoId is undefined",
			);
			...
			assertEqual(
				loaded!.lanes[0].laneSessionId,
				"orch-lane-1",
				"v1 upconverted: lane-1 sessionName preserved",
			);
...

		{
			...
			assertEqual(
				parsedAfter.mode,
				undefined,
				"on-disk after load: mode field absent (v1 had no mode)",
			);
...

		{
			...
			assertEqual(
				loaded!.schemaVersion,
				BATCH_STATE_SCHEMA_VERSION,
				"v2 workspace: schemaVersion is 2",
			);
...

		{
			...
			assertEqual(
				...
				BATCH_STATE_SCHEMA_VERSION,
				"round-trip: reloaded schemaVersion is 2",
			);
	...
	try {
		{
			...
			assert(
				existsSync(batchStatePath(persistTestRoot)),
				"state file exists after batch-start persist",
			);
...

		{
			...
			assert(
				loaded!.lastError!.message.includes("Merge failed"),
				"lastError message includes merge failure",
			);
...

		{
			...
			assert(
				state2.errors[0].includes("State persistence failed"),
				"error message mentions persistence failure",
			);
		...
		{
			console.log(
				"  ▸ serialization omits repo fields for repo-mode state (no repo fields on lanes/tasks)",
			);
			if (!existsSync(join(persistTestRoot, ".pi"))) {
				mkdirSync(join(persistTestRoot, ".pi"), { recursive: true });
			}
...

		{
			...
			const discovery = {
				pending: new Map([
					...
					[
						"WS-020",
						{ taskFolder: "/tasks/WS-020", promptRepoId: "frontend", resolvedRepoId: "frontend" },
					],
				]),
			};
			...
			assertEqual(
				ws010!.resolvedRepoId,
				"api",
				"WS-010 resolvedRepoId from serialization (allocated)",
			);
	...
	function minimalPersistedState(
		overrides?: Partial<PersistedBatchStateForTest>,
	): PersistedBatchStateForTest {
		return {
			schemaVersion: 2,
			phase: "executing",
...

	{
		...
		assertEqual(
			...
			"paused-corrupt",
			"recommend paused-corrupt for invalid state",
		);
	...
	{
		console.log(
			"  ▸ all tasks in wave 0 done → resumeWaveIndex=1, future-wave pending task remains pending",
		);
		const state = minimalPersistedState({
			wavePlan: [["T1", "T2"], ["T3"]],
			tasks: [
		...
		assert(
			point.pendingTaskIds.includes("T3"),
			"T3 is pending for execution (never-started future-wave task)",
		);
...

	{
		...
		assert(
			typeof forceKill === "string" && forceKill.length > 0,
			"abortGracefulForceKill returns string",
		);
		...
		assert(
			typeof hardStarting === "string" && hardStarting.length > 0,
			"abortHardStarting returns string",
		);
		...
		assert(
			typeof hardComplete === "string" && hardComplete.length > 0,
			"abortHardComplete returns string",
		);
		assert(hardComplete.includes("4"), "abortHardComplete includes kill count");

		const partialFailure = messages.abortPartialFailure(3);
		assert(
			typeof partialFailure === "string" && partialFailure.length > 0,
			"abortPartialFailure returns string",
		);
	...
	function detectMixedOutcomeLanes(
		laneResults: TestLaneExecutionResult[],
	): TestLaneExecutionResult[] {
...

	{
		...
		assert(
			result.failureReason!.includes("both succeeded and failed"),
			"failure reason explains mixed outcomes",
		);
	}
	...
	{
		console.log(
			"  ▸ merge failure + pause policy → preserveWorktrees=true, phase=paused, persist before cleanup",
		);
	...
	{
		console.log(
			"  ▸ merge failure + abort policy → preserveWorktrees=true, phase=stopped, persist before cleanup",
		);

		const result = simulateMergeFailureHandling(
			"failed",
			"abort",
			1,
			"BUILD_FAILURE on verification",
		);
	...
	{
		console.log(
			"  ▸ clean completion (no merge failure) → preserveWorktrees=false, cleanup proceeds",
		);
	...
	// Verify the source code actually has the cleanup suppression logic
	{
		...
		assert(
			source.includes("skipping final cleanup to preserve worktrees"),
			"source logs cleanup skip reason",
		);
	...
	function parseMergeResultCore(filePath: string): any {
		...
		return {
			...
			verification: {
				...
				output:
					typeof parsed.verification.output === "string"
	...
	try {
...

		{
			...
			assertEqual(
				...
				"",
				"missing verification.output defaults to empty string",
			);
		}
	} finally {
		try {
	...
	// Verify the source code has the retry logic and unknown status handling
	{
		...
		assert(
			source.includes(`parsed.status = "BUILD_FAILURE"`),
			"source maps unknown status to BUILD_FAILURE",
		);
		assert(
			source.includes("MERGE_RESULT_MISSING_FIELDS"),
			"source uses MERGE_RESULT_MISSING_FIELDS error code",
		);
...

	{
		...
		try {
			...
			assertEqual(
				...
				1,
				"1 task marked failed (E2E-005: pending + dead session)",
			);
...

	{
		...
		try {
			...
			assertEqual(
				afterParsed.baseBranch,
				undefined,
				"on-disk: baseBranch still absent (v1 has no baseBranch)",
			);
...

	{
		...
		try {
			...
			assertEqual(
				afterParsed.schemaVersion,
				BATCH_STATE_SCHEMA_VERSION,
				"on-disk: v2 after explicit save",
			);
...

	{
		...
		try {
			...
			assertEqual(
				loaded!.schemaVersion,
				BATCH_STATE_SCHEMA_VERSION,
				"v2 repo-mode load: schemaVersion is 2",
			);
...

	{
		...
		try {
			...
			assertEqual(
				loaded!.tasks[1].repoId,
				undefined,
				"v2 workspace-mode load: task[1].repoId is undefined",
			);
			assertEqual(
				loaded!.tasks[1].resolvedRepoId,
				"frontend",
			...
			assertEqual(
				loaded!.lanes[1].repoId,
				"frontend",
				"v2 workspace-mode load: lane[1].repoId is 'frontend'",
			);
		} finally {
			try {
	...
	{
		console.log(
			"  ▸ v1 upconverted state is usable for resume flow (loadBatchState → reconcile → resume)",
		);
		...
		try {
			...
			assertEqual(
				orphanResult.recommendedAction,
				"resume",
				"v1 resume: orphan detection recommends resume",
			);
		} finally {
			try {
	...
	function workspacePersistedState(
		overrides?: Partial<PersistedBatchStateForTest>,
	): PersistedBatchStateForTest {
		return {
			schemaVersion: 2,
...

	{
		...
		assertEqual(
			result[1].action,
			"mark-failed",
			"WS-002: mark-failed (dead session, no DONE, no worktree)",
		);
	...
	{
		console.log(
			"  ▸ workspace v2: .DONE in one repo + dead session in another → mark-complete vs mark-failed",
		);
		const state = workspacePersistedState();
	...
	{
		console.log(
			"  ▸ workspace v2: worktree exists vs missing split across repos → re-execute vs mark-failed",
		);
		const state = workspacePersistedState();
	...
	{
		console.log(
			"  ▸ resolveRepoRoot: v2 lanes get correct repo root, v1/undefined lanes get default root",
		);
		const wsConfig = {
			repos: new Map([
				["api", { path: "/repos/api" }],
...

	{
		...
		assertEqual(
			...
			0,
			"workspace: resume from wave 0 (mark-failed NOT done for wave-skip)",
		);
	}

	{
		...
		assertEqual(
			point.resumeWaveIndex,
			0,
			"v1: resume from wave 0 (mark-failed NOT done for wave-skip)",
		);
...

	{
		...
		assertEqual(
			ws001!.liveStatus,
			"pending",
			"workspace: WS-001 live status pending (will be re-executed)",
		);
...

	{
		...
		const wave1Tasks = state.wavePlan[1].filter(
			(taskId: string) =>
				!completedSet.has(taskId) && !failedSet.has(taskId) && !blockedTaskIds.has(taskId),
		);
...

	{
		...
		const wave1BlockedCount = state.wavePlan[1].filter((taskId: string) =>
...

	{
		...
		assertEqual(
			point.resumeWaveIndex,
			1,
			"v1 fallback: resumes from wave 1 (mark-failed NOT done for wave-skip)",
		);
...

	{
		...
		assertEqual(
			reconciled[0].action,
			"mark-complete",
			"mark-complete-always: action is mark-complete",
		);
		assertEqual(
			reconciled[0].persistedStatus,
			"running",
			"mark-complete-always: persisted was running",
		);
	...
	{
		console.log(
			"  ▸ reconstructAllocatedLanes: v1 lanes (no repoId) produce lanes without repoId field",
		);
		const v1Lanes = [
			{
				laneNumber: 1,
	...
	{
		console.log(
			"  ▸ serializeBatchState: reconstructed lanes preserve repo attribution through serialization",
		);
		const persistedLanes = [
			{
				laneNumber: 1,
	...
	// 2.7: Checkpoint attribution invariants across persistence triggers
	{
		...
		const json1 = serializeBatchState(
			reconcileState,
			[["T1"], ["T2"]],
			reconstructed,
			reconcileOutcomes,
		...
		assertEqual(
			t2.resolvedRepoId,
			"frontend",
			"wave checkpoint: T2 resolvedRepoId from allocated task",
		);
	}
	...
	{
		console.log(
			"  ▸ collectAllRepoRoots: repos from resumed wave allocation are included in cleanup set",
		);
		const wsConfig = {
			repos: new Map<string, { path: string }>([
				["api", { path: "/repos/api" }],
		...
		const roots = collectAllRepoRoots(
			[persistedLaneSources, newAllocatedSources],
			"/default",
			wsConfig,
		);
	...
	{
		console.log(
			"  ▸ v1 fallback: reconstructAllocatedLanes + collectAllRepoRoots unchanged for v1 state",
		);
		const v1Lanes = [
			{
				laneNumber: 1,
	...
	{
		console.log(
			"  ▸ checkpoint round-trip: serialize → validate → lanes[].repoId + tasks[].repoId survive",
		);
		...
		assertEqual(
			reReconstruct[0].repoId,
			"api",
			"re-reconstruct: repoId preserved across pause/resume",
		);
	}
	...
	{
		console.log(
			"  ▸ reconstructAllocatedLanes: persistedTasks carries repo fields for archived tasks",
		);
		const persistedLanes = [
			{
				laneNumber: 1,
	...
	{
		console.log(
			"  ▸ reconstructAllocatedLanes: without persistedTasks, task stubs are null (backward compat)",
		);
		const persistedLanes = [
			{
				laneNumber: 1,
	...
	{
		console.log(
			"  ▸ blocked counter: persisted-blocked tasks in unvisited waves counted at resume init",
		);
	...
	{
		console.log(
			"  ▸ mixed-repo checkpoint: tasks from 2 repos preserve attribution through serialize",
		);
		const persistedLanes = [
			{
				laneNumber: 1,
